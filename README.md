...
for scenario_name, scenario_list in scenarios.items():
    ...
    for file in scenarios[scenario_name]:
        ...
        # Table 1: Summarised EWR results for the entire timeseries
        temp_summarised_ewr_results = ewr_sh.get_ewr_results()
        summarised_ewr_results = pd.concat([summarised_ewr_results,temp_summarised_ewr_results], axis = 0)
        ...
        # Table 4: Inverse of Table 3 showing the interevent periods
        temp_all_interevent_periods = ewr_sh.get_all_interEvents()
        all_interevent_periods = pd.concat([all_interevent_periods, temp_all_interevent_periods], axis = 0)

        # Table 5: All events details that also meet the duration requirement:
        temp_all_successful_events = ewr_sh.get_all_successful_events()
        all_successful_events = pd.concat([all_successful_events, temp_all_successful_events], axis = 0)

        # Table 6: Inverse of Table 5 showing the interevent periods:
        temp_all__interevent_periods = ewr_sh.get_all_successful_interEvents()
        all_successful_interevent_periods = pd.concat(all_succesful_iterevent_periods, temp_all_successful_interevent_periods], axis = 0)
        

    # Optional code to output results to csv files:
    summarised_ewr_results.to_csv(scenario_name + 'all_results.csv')
    yearly_ewr_results.to_csv(scenario_name + 'yearly_ewr_results.csv')
    all_events.to_csv(scenario_name + 'all_events.csv')
    all_interevent_periods.to_csv(scenario_name + 'all_interevents.csv')
    ...
    summarised_ewr_results_dict[scenario_name] = summarised_ewr_results
    yearly_results_dict[scenario_name] = yearly_ewr_results
    all_events_dict[scenario_name] = all_events_dict
    all_interevent_periods_dict[scenario_name] = all_interevent_periods
    all_successful_Events_dict[scenario_name] = all_successful_events
    all_successful_interEvents_dict[scenario_name] = all_successful_interevent_periods

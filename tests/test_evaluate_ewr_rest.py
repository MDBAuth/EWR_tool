...
def test_flow_check(
    ...
    for year in all_events:
        for i, event in enumerate(all_events[year]):
            assert (
                event == expected_all_events[year][i]
...
@pytest.mark.parametrize(
    "test_ID, flows,expected_all_events",
    [
        (
            "test_case_1",
            np.array(
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                2012: [
                    [(date(2013, 5, 17) + timedelta(days=i), 10) for i in range(45)]
                ],
                2013: [],
                2014: [
                    [(date(2015, 6, 21) + timedelta(days=i), 10) for i in range(10)]
                ],
                2015: [
                    [(date(2016, 5, 16) + timedelta(days=i), 10) for i in range(46)]
                ],
            },
        ),
        (
...
def test_lowflow_calc(
    test_ID, EWR_info, flows, dates, masked_dates, expected_all_events
):
    """
    ...
    for year in all_events:
        ...
        for i, event in enumerate(all_events[year]):
            assert (
                event == expected_all_events[year][i]
            ), f"event list does not match expected_all_events[year][i] {test_ID}"
...
@pytest.mark.parametrize(
    "flows,expected_all_events,expected_all_no_events",
    [
        (
            np.array(
...
def test_ctf_calc_anytime(flows, expected_all_events, expected_all_no_events):
    ...
    def test_ctf_calc_short_events():
        ...
        # Send to test function and then test
        all_events, durations = evaluate_EWRs.ctf_calc_anytime(
...
@pytest.mark.parametrize(
    "EWR_info,flows,expected_all_events,expected_all_no_events",
    [
        (
            {
                "min_volume": 120,
...
@pytest.mark.parametrize(
    "EWR_info,iteration,flows,event,all_events,all_no_events,expected_all_events,expected_event",
    [
        (
            {
                "min_volume": 100,
...
def test_volume_check(
    ...
    assert event == expected_event
    assert expected_all_events is not None
...
@pytest.mark.parametrize(
    "EWR_info,iteration,flow,level,event,all_events,all_no_events,weirpool_type,level_change,total_event,expected_all_events,expected_event",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,flows,levels,weirpool_type,expected_all_events,expected_all_no_events",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,iteration,level,level_change,event,all_events,all_no_events,total_event,expected_all_events,expected_event",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,levels,expected_all_events,expected_all_no_events",
    [
        (
            {
                "min_flow": 5,
...
def test_component_pull_nest(gauge, PU, EWR, component, expected_result):
    ...
    """
    EWR_table, bad_EWRs = data_inputs.get_EWR_table(
        "./unit_testing_files/MURRAY_MDBA_update_nest.csv"
    )

    assert (
        evaluate_EWRs.component_pull(EWR_table, gauge, PU, EWR, component)
        == expected_result
    )


@pytest.mark.parametrize(
    "EWR_info,iteration,flow,flow_percent_change,event,all_events,all_no_events,total_event,expected_all_events,expected_event",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,iteration,flow,level,event,all_events,all_no_events,weirpool_type,levels,total_event,expected_all_events,expected_event",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,iteration,flow,level,event,all_events,all_no_events,level_change,levels,total_event,expected_all_events,expected_event",
    [
        (
            {
                "min_flow": 5,
...
@pytest.mark.parametrize(
    "EWR_info,flows,event,all_events,all_no_events,expected_all_events,expected_event,flow_date",
    [
        (
            {

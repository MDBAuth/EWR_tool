...
import data_inputs
import evaluate_EWRs
import summarise_results
from MDBA_Gauge_Getter.mdba_gauge_getter import gauge_getter as gg


def observed_handler(gauges: list[str], dates, allowance, climate) -> tuple[dict, pd.DataFrame]:
    ...
    # Calculate EWRs
    detailed_results, gauge_results, gauge_events, detailed_events = {}, {}, {}, {}
    # detailed_results = {}
    # gauge_results = {}
    # gauge_events = {}
    # detailed_events = {}
...
def categorise_gauges(gauges: list) -> tuple[list[str], list[str]]:
    ...
    unique_flow_gauges, unique_level_gauges = [], []
    ...
    for gauge in gauges:
        ...
        unique_flow_gauges.append(flow_gauges)
        unique_level_gauges.append(level_gauges)

    assert unique_flow_gauges
    assert unique_level_gauges

    return unique_flow_gauges[0], unique_level_gauges[0]
...
def observed_cleaner(input_df: pd.DataFrame, dates) -> pd.DataFrame:
